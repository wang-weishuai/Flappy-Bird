...
class Game():
    ...
    def draw(self):
        ...
        oled.text('%d' % (self.obstacle_1.score + self.obstacle_2.score), WIDTH // 2 - 12, 0)
        oled.show()
########################################
    def _clicked(self):
        sounds = sound.read()
        if sounds < 1000 and not self.pressed:
            self.pressed = True
            return True
        elif sounds >= 1000 and self.pressed:
            self.pressed = False
        return False                 #又改回了最初版
############################################
    # 开机画面
    def game_start(self):
        oled.fill(0)
        ...
        oled.text('Record: ' + '%d' % self.high_score, WIDTH // 2 - 44, HEIGHT // 2 - 6)
        oled.show()
        self.game_state = 1
